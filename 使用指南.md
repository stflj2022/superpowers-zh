...
**项目**: https://github.com/stflj2022/superpowers-zh
...
```bash
/plugin install /path/to/superpowers-zh
```
...
> 注：`~` 代表用户主目录（Windows: `C:\Users\<用户名>`，Linux/macOS: `/home/<用户名>`）
...
```bash
# 创建缓存目录
mkdir -p "~/.claude/plugins/cache/superpowers-zh-dev/superpowers-zh/1.0.0"

# 复制所有文件
cp -r /path/to/superpowers-zh/* ~/.claude/plugins/cache/superpowers-zh-dev/superpowers-zh/1.0.0/

# 单独复制隐藏目录（关键！）
cp -r /path/to/superpowers-zh/.claude-plugin ~/.claude/plugins/cache/superpowers-zh-dev/superpowers-zh/1.0.0/
```
...
"superpowers-zh-dev": {
  "source": {
    "source": "directory",
    "path": "/path/to/superpowers-zh"
  },
  "installLocation": "/path/to/superpowers-zh",
...
"superpowers-zh@superpowers-zh-dev": [{
  "scope": "user",
  "installPath": "/home/<用户名>/.claude/plugins/cache/superpowers-zh-dev/superpowers-zh/1.0.0",
  "version": "1.0.0",
...
> Windows 用户请使用：`C:\\Users\\<用户名>\\.claude\\plugins\\cache\\superpowers-zh-dev\\superpowers-zh\\1.0.0`
